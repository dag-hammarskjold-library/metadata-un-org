...
def rebuild_index(datafile):
    ...
    if es_con.indices.exists(index=index_name):
...
SELECT * WHERE {
    ...
    for tcode, uri in graph.query(querystring):
        this_uri = uri
        this_tcode = tcode
        print('Processing new term:')
        print(f'uri: {this_uri}, tcode: {this_tcode}')
        doc = {"uri": this_uri}
        for lang in CONFIG.LANGUAGES:
            pref_labels = []
            pref_labels = [o for s,p,o in graph.triples((URIRef(this_uri), SKOS.prefLabel, None)) if o.language == lang]
            doc.update({"labels_{}".format(lang): pref_labels})

            alt_labels = [o for s,p,o in graph.triples((URIRef(this_uri), SKOS.altLabel, None)) if o.language == lang]
            doc.update({"alt_labels_{}".format(lang): alt_labels})

            if tcode is not None:
                doc.update({"tcode": this_tcode}) 

            hidden_labels = [o for s,p,o in graph.triples((None, SKOS.hiddenLabel, None)) if o.language == lang]
